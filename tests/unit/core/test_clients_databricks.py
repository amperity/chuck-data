...
class TestBuildLibraries:
    ...
    def test_redshift_adds_redshift_connector(self, databricks_api_client):
        """aws_redshift data_provider includes the Redshift connector."""
        libs = databricks_api_client._build_libraries(data_provider="aws_redshift")
        coordinates = [lib["maven"]["coordinates"] for lib in libs if "maven" in lib]
        assert any("spark-redshift" in c for c in coordinates)

    def test_default_excludes_all_connectors(self, databricks_api_client):
        """Default (no data_provider) does not include any connector."""
