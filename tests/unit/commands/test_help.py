...
def test_help_command_content_real_logic():
    ...
    found_indicators = [indicator for indicator in expected_content_indicators 
                       if indicator.lower() in help_text.lower()]
    
    assert len(found_indicators) > 0, f"Expected to find command indicators in help text: {help_text[:200]}..."
