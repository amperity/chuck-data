...
def test_handle_status_with_valid_connection_real_logic(databricks_client_stub):
    ...
    with tempfile.NamedTemporaryFile() as tmp:
        ...
        with patch("chuck_data.config._config_manager", config_manager):
            with patch("chuck_data.commands.status.validate_all_permissions") as mock_permissions:
...
def test_handle_status_with_permission_error_real_logic(databricks_client_stub):
    ...
    assert result.success
    assert "Permission denied" in result.data["connection_status"] or "error" in result.data["connection_status"]
...
def test_handle_status_with_partial_config_real_logic(databricks_client_stub):
    ...
    assert result.data["active_catalog"] is None or isinstance(result.data["active_catalog"], str)
    assert result.data["connection_status"] == "Connected (client present)."
...
def test_handle_status_real_config_integration():
    ...
    with tempfile.NamedTemporaryFile() as tmp:
        ...
        config_manager.update(workspace_url="https://second.databricks.com")  # Update workspace
    ...
    assert result.success
    assert result.data["workspace_url"] == "https://second.databricks.com"  # Latest update
