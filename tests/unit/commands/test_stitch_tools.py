...
def test_missing_params(databricks_client_stub, llm_client_stub):
    ...
    result = _helper_setup_stitch_logic(databricks_client_stub, llm_client_stub, "", "test_schema")
...
def test_volume_list_error(databricks_client_stub, llm_client_stub, mock_pii_scan_results):
    """Test handling when listing volumes fails."""
    # Set up PII scan to succeed by providing tables with PII
    databricks_client_stub.add_table("test_catalog", "test_schema", "customers", 
                                      columns=[{"name": "email", "type_name": "STRING"}])
    databricks_client_stub.add_table("test_catalog", "test_schema", "orders",
...
def test_volume_create_error(databricks_client_stub, llm_client_stub, mock_pii_scan_results):
...
def test_no_tables_with_pii(databricks_client_stub, llm_client_stub, mock_pii_scan_results):
    """Test handling when no tables with PII are found."""
    # Set up tables with no PII (LLM returns no semantic tags)
    databricks_client_stub.add_table("test_catalog", "test_schema", "metrics", 
...
def test_missing_amperity_token(databricks_client_stub, llm_client_stub, mock_pii_scan_results):
...
def test_amperity_init_script_error(databricks_client_stub, llm_client_stub, mock_pii_scan_results):
...
def test_versioned_init_script_upload_error(databricks_client_stub, llm_client_stub, mock_pii_scan_results):
    ...
    with tempfile.NamedTemporaryFile() as tmp:
        ...
        with patch("chuck_data.config._config_manager", config_manager):
            ...
            with patch("chuck_data.commands.stitch_tools._helper_upload_cluster_init_logic") as mock_upload:
                mock_upload.return_value = {"error": "Failed to upload versioned init script"}
...
def test_successful_setup(databricks_client_stub, llm_client_stub, mock_pii_scan_results):
...
def test_unsupported_types_filtered(databricks_client_stub, llm_client_stub, mock_pii_scan_results_with_unsupported):
    ...
    with tempfile.NamedTemporaryFile() as tmp:
        ...
        with patch("chuck_data.config._config_manager", config_manager):
            ...
            databricks_client_stub.add_table("test_catalog", "test_schema", "geo_data",
                                              columns=[
            ...
            with patch("chuck_data.commands.stitch_tools._helper_upload_cluster_init_logic") as mock_upload:
                ...
                assert "string" in config_content.lower(), "Config should contain supported type: string"
...
def test_all_columns_unsupported_types(databricks_client_stub, llm_client_stub):
    ...
    with tempfile.NamedTemporaryFile() as tmp:
        ...
        with patch("chuck_data.config._config_manager", config_manager):
            ...
            databricks_client_stub.add_table("test_catalog", "test_schema", "complex_data",
                                              columns=[
                                                  {"name": "metadata", "type_name": "STRUCT"},
                                                  {"name": "tags", "type_name": "ARRAY"},
                                                  {"name": "location", "type_name": "GEOGRAPHY"}
