...
class TestRedshiftAPIClientInitialization:
    ...
    def test_initialization_with_all_optional_parameters(self):
        ...
        assert client.emr_cluster_id == "j-XXXXXXXXXXXXX"
...
class TestSQLExecution:
    ...
    def test_execute_sql_with_wait_success(self, mock_sleep, mock_boto3):
        ...
        mock_redshift_data = Mock()
        mock_redshift_data.execute_statement.return_value = {"Id": "statement-123"}
        mock_redshift_data.describe_statement.return_value = {"Status": "FINISHED"}
...
class TestEMROperations:
    """Test EMR operations."""

    @patch("chuck_data.clients.redshift.boto3")
    def test_list_emr_clusters_success(self, mock_boto3):
        """Test successful EMR cluster listing."""
        mock_emr = Mock()
        mock_emr.list_clusters.return_value = {
            "Clusters": [
                {"Id": "j-CLUSTER1", "Name": "Cluster 1"},
                {"Id": "j-CLUSTER2", "Name": "Cluster 2"},
            ]
        }
        mock_boto3.client.return_value = mock_emr
        ...
        clusters = client.list_emr_clusters()

        assert len(clusters) == 2
        assert clusters[0]["Id"] == "j-CLUSTER1"

    @patch("chuck_data.clients.redshift.boto3")
    def test_verify_emr_cluster_running_success(self, mock_boto3):
        """Test successful EMR cluster verification."""
        mock_emr = Mock()
        ...
        client = RedshiftAPIClient(
            ...
            region="us-west-2",
            cluster_identifier="test-cluster",
            emr_cluster_id="j-TESTCLUSTER",
        )

        state = client.verify_emr_cluster_running()

        assert state == "RUNNING"

    @patch("chuck_data.clients.redshift.boto3")
    def test_verify_emr_cluster_without_cluster_id(self, mock_boto3):
        """Test EMR cluster verification without cluster ID."""
        mock_boto3.client.return_value = Mock()
        ...
        with pytest.raises(ValueError) as exc_info:
            client.verify_emr_cluster_running()

        assert "EMR cluster ID not configured" in str(exc_info.value)

    @patch("chuck_data.clients.redshift.boto3")
    def test_submit_emr_spark_job_success(self, mock_boto3):
        """Test successful EMR Spark job submission."""
        mock_emr = Mock()
        mock_emr.describe_cluster.return_value = {
            "Cluster": {"Status": {"State": "RUNNING"}}
        }
        mock_emr.add_job_flow_steps.return_value = {"StepIds": ["step-123"]}
        mock_boto3.client.return_value = mock_emr
        ...
        result = client.submit_emr_spark_job(
            "s3://bucket/job.jar", "com.example.Main", ["arg1", "arg2"]
        )

        assert result["cluster_id"] == "j-TESTCLUSTER"
        assert result["step_id"] == "step-123"

    @patch("chuck_data.clients.redshift.boto3")
    def test_submit_emr_spark_job_without_cluster_id(self, mock_boto3):
        """Test EMR job submission without cluster ID."""
        mock_boto3.client.return_value = Mock()
        ...
        with pytest.raises(ValueError) as exc_info:
            client.submit_emr_spark_job("s3://bucket/job.jar", "com.example.Main", [])

        assert "EMR cluster ID not configured" in str(exc_info.value)

    @patch("chuck_data.clients.redshift.boto3")
    def test_submit_emr_spark_job_cluster_not_running(self, mock_boto3):
        """Test EMR job submission with cluster not running."""
        mock_emr = Mock()
        mock_emr.describe_cluster.return_value = {
        ...
        assert "not in RUNNING/WAITING state" in str(exc_info.value)

    @patch("chuck_data.clients.redshift.boto3")
    def test_get_emr_step_status_success(self, mock_boto3):
        """Test successful EMR step status retrieval."""
        mock_emr = Mock()
        mock_emr.describe_step.return_value = {
            "Step": {"Id": "step-123", "Status": {"State": "COMPLETED"}}
        }
        mock_boto3.client.return_value = mock_emr
        ...
        step = client.get_emr_step_status("step-123")
