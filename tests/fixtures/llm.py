...
class LLMClientStub:
    ...
    def chat(self, messages, model=None, tools=None, stream=False, tool_choice="auto"):
        ...
        if hasattr(self, '_pii_chat_override') and self._pii_chat_override:
            return self._pii_chat_override(messages, model=model, tools=tools, stream=stream, tool_choice=tool_choice)
                ...
                try:
                    ...
                    for pattern in patterns:
                        ...
                        if found:
                            ...
                            columns_in_message = [c for c in found if c != "column_name"]
                            break
            
            
            # Create a map of column names to semantic tags
            pii_map = {r["column"]: r.get("semantic") for r in self.pii_detection_results}
            
            # Build response for ONLY the columns found in the message
            for col_name in columns_in_message:
                json_response.append({
                    "name": col_name,
                    "semantic": pii_map.get(col_name)
                })
