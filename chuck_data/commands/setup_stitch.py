...
def _snowflake_prepare_manifest(
    ...
    **kwargs,
    ...
    if isinstance(storage_provider, DatabricksVolumeStorage):
        # Databricks Volumes require a full catalog.schema.volume path prefix
        from chuck_data.config import get_active_catalog

        d_catalog = get_active_catalog()
        d_vol_schema = get_active_schema()
        if not d_catalog or not d_vol_schema:
            return {
                "success": False,
                "error": (
                    "No active Databricks catalog/schema set for volume storage. "
                    "Use /select-catalog and /select-schema to configure where the "
                    "Stitch manifest will be stored in Databricks Volumes."
                ),
            }
...
def _snowflake_execute_job_launch(
    ...
    **kwargs,
    ...
    try:
        ...
        # Databricks compute
        console.print(
            f"[{INFO_STYLE}]Note: The Snowflake Spark connector must be installed on your "
            f"Databricks cluster as a Maven library:\n"
            f"  net.snowflake:spark-snowflake_2.12:2.16.0-spark_3.5[/{INFO_STYLE}]"
        )
        return CommandResult(
            True,
            message=f"Stitch job for {database}.{schema} submitted (run_id: {run_id}).",
            data={
                "run_id": run_id,
                "job_id": job_id,
                "config_path": upload_path,
