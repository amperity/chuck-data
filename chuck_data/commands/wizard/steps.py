    ...
    def handle_input(self, input_text: str, state: WizardState) -> StepResult:
        ...
        try:
            ...
            if state.data_provider == "databricks":
                ...
                # Snowflake data + Databricks compute: collect the Databricks
                # catalog and schema where the chuck volume will be created
                next_step = WizardStep.DATABRICKS_VOLUME_CATALOG_INPUT
                message = (
                    "Databricks credentials configured. "
                    "Now specify where Stitch artifacts will be stored."
                )
            else:
...
class DatabricksVolumeCatalogInputStep(SetupStep):
    """Collect the Databricks catalog where the chuck volume will be created.

    Only shown when data_provider == "snowflake" and compute_provider == "databricks".
    The chuck volume in this catalog.schema is used to store Stitch manifests
    and cluster init scripts so Databricks clusters can read them at startup.
    """

    def get_step_title(self) -> str:
        return "Databricks Volume Catalog"

    def get_prompt_message(self, state: WizardState) -> str:
        return (
            "Stitch manifests and init scripts will be stored in a Databricks Volume.\n"
            "Enter the Databricks Unity Catalog where a 'chuck' volume will be created:\n"
            "Catalog name:"
        )

    def handle_input(self, input_text: str, state: WizardState) -> StepResult:
        catalog = input_text.strip()
        if not catalog:
            return StepResult(
                success=False,
        ...
        try:
            ...
            get_config_manager().update(volume_catalog=catalog)
            return StepResult(
                success=True,
                message=(
                    f"Catalog '{catalog}' saved. "
                    "Now enter the schema within that catalog."
                ),
                next_step=WizardStep.DATABRICKS_VOLUME_SCHEMA_INPUT,
                action=WizardAction.CONTINUE,
                data={"volume_catalog": catalog},
            )
        except Exception as e:
            logging.error(f"Error saving volume catalog: {e}")
            return StepResult(
                success=False,
                message=f"Error saving catalog: {str(e)}",
                action=WizardAction.RETRY,
            )


class DatabricksVolumeSchemaInputStep(SetupStep):
    """Collect the Databricks schema where the chuck volume will be created.

    After saving, attempts to create the chuck volume if it does not already
    exist, so the user gets immediate feedback on any permissions issues.
    """

    def get_step_title(self) -> str:
        return "Databricks Volume Schema"

    def get_prompt_message(self, state: WizardState) -> str:
        catalog = state.volume_catalog or "<catalog>"
        return (
            f"Enter the schema within '{catalog}' where the 'chuck' volume will live.\n"
            "Schema name:"
        )

    def handle_input(self, input_text: str, state: WizardState) -> StepResult:
        schema = input_text.strip()
        if not schema:
            return StepResult(
                success=False,
        ...
        catalog = state.volume_catalog
        if not catalog:
            return StepResult(
                success=False,
                message="Catalog not set. Please restart the wizard.",
                action=WizardAction.EXIT,
            )

        # Try to create (or confirm existence of) the chuck volume now so the
        # user gets early feedback on permission issues before running Stitch.
        try:
            service = get_chuck_service()
            if service and service.client:
                dbx = service.client
                volumes_resp = dbx.list_volumes(
                    catalog_name=catalog, schema_name=schema
                )
                volume_names = {v.get("name") for v in volumes_resp.get("volumes", [])}
                if "chuck" not in volume_names:
                    dbx.create_volume(
                        catalog_name=catalog, schema_name=schema, name="chuck"
                    )
                    volume_note = f"Created 'chuck' volume in {catalog}.{schema}."
                else:
                    volume_note = (
                        f"'chuck' volume already exists in {catalog}.{schema}."
                    )
            else:
                volume_note = (
                    "Could not verify volume (no active client); "
                    "the volume will be created when Stitch runs."
                )
        except Exception as e:
            logging.warning(f"Could not prepare chuck volume: {e}")
            # Non-fatal: save the config and let the user proceed; the
            # volume creation will be retried when Stitch is launched.
            volume_note = (
                f"Warning: could not create 'chuck' volume ({e}). "
                "It will be created when Stitch runs."
            )

        try:
            from chuck_data.config import get_config_manager

            get_config_manager().update(volume_schema=schema)
            return StepResult(
                success=True,
                message=(
                    f"Volume location set to {catalog}.{schema}.chuck. "
                    f"{volume_note} "
                    "Select your LLM provider."
                ),
                next_step=WizardStep.LLM_PROVIDER_SELECTION,
                action=WizardAction.CONTINUE,
                data={"volume_schema": schema},
            )
        except Exception as e:
            logging.error(f"Error saving volume schema: {e}")
            return StepResult(
                success=False,
...
def create_step(step_type: WizardStep, validator: InputValidator) -> SetupStep:
    ...
    step_map = {
        ...
        # Databricks Volume location (Snowflake data + Databricks compute)
        WizardStep.DATABRICKS_VOLUME_CATALOG_INPUT: DatabricksVolumeCatalogInputStep,
        WizardStep.DATABRICKS_VOLUME_SCHEMA_INPUT: DatabricksVolumeSchemaInputStep,
    }
